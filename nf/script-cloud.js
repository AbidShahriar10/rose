...
const makeItRain = () => {
  ...
  const cloudRight = (cloud.getBoundingClientRect().width + cloudLeft) - 20;
	const tearPosRandom = (Math.floor(Math.random() * (cloudRight - cloudLeft + 1)) + cloudLeft) + 'px';
	const tearPosVert = (cloud.getBoundingClientRect().bottom - cloud.getBoundingClientRect().height) + 'px';
